...
namespace SPH
{
	...
	protected:
		...
		void splitFluidBlockParticles(unsigned int sourceFluidIndex, std::map<std::string, unsigned int>& fluidIDs, std::vector<std::vector<Vector3r>>& fluidParticles, std::vector<std::vector<Vector3r>>& fluidVelocities, std::vector<std::vector<unsigned int>>& fluidObjectIds);
		void createEmitters();
