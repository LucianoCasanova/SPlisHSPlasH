...
#include "SPlisHSPlasH/VdWSPH/TimeStepVdWSPH.h"
...
int Simulation::ENUM_SIMULATION_VdWSPH = -1;
int Simulation::ENUM_SIMULATION_ISPH = -1;
...
void Simulation::initParameters()
{
	...
	enumParam->addEnumValue("VdWSPH", ENUM_SIMULATION_VdWSPH);
	enumParam->addEnumValue("ISPH", ENUM_SIMULATION_ISPH);
...
void Simulation::initKernels()
{
	...
	CubicKernelvdW::setRadius(m_supportRadius);
#ifdef USE_AVX
...
void Simulation::setSimulationMethod(const int val)
{
	...
	else if (method == SimulationMethods::VdWSPH)
	{
		m_timeStep = new TimeStepVdWSPH();
		m_timeStep->init();
		setValue(Simulation::KERNEL_METHOD, Simulation::ENUM_KERNEL_CUBIC);
		setValue(Simulation::GRAD_KERNEL_METHOD, Simulation::ENUM_GRADKERNEL_CUBIC);
	}
	else if (method == SimulationMethods::ISPH)
	{
		m_timeStep = new TimeStepISPH();
		m_timeStep->init();
		setValue(Simulation::KERNEL_METHOD, Simulation::ENUM_KERNEL_CUBIC);
		setValue(Simulation::GRAD_KERNEL_METHOD, Simulation::ENUM_GRADKERNEL_CUBIC);
	}
