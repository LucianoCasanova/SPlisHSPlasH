	...
	*/
	class FluidModel : public GenParam::ParameterObject
	{
		...
		protected:
			...
			std::vector<Real> m_numberDensity;
		...
		public:
			...
			FORCE_INLINE const Real& getNumberDensity(const unsigned int i) const
			{
				return m_numberDensity[i];
			}

			FORCE_INLINE Real& getNumberDensity(const unsigned int i)
			{
				return m_numberDensity[i];
			}

			FORCE_INLINE void setNumberDensity(const unsigned int i, const Real& val)
			{
				m_numberDensity[i] = val;
			}
