...
namespace SPH
{
	...
	*/
	...
	/** \brief Cubic spline kernel with double radius.
*/
	class CubicKernelvdW
	{
	protected:
		static Real m_radius;
		static Real m_k;
	...
	public:
		...
		static void setRadius(Real val)
		{
			m_radius = static_cast<Real>(2.0) * val;
			const Real pi = static_cast<Real>(M_PI);
	...
	/** \brief Gaussian with 3 sigma
*/
	class Gaussian3
	{
	protected:
		Real m_radius; // support radius
		Real m_k;
		Real m_l;
		Real m_W_zero;
	public:
		...
		void setRadius(Real val)
		{
			m_radius =  val;
			const Real pi = static_cast<Real>(M_PI);
			const Real sqrtPi = static_cast<Real>(2.0) / static_cast<Real>(M_2_SQRTPI);
			const Real pi_3_2 = sqrtPi * sqrtPi * sqrtPi;

			const Real h3 = m_radius * m_radius * m_radius;
			m_k = static_cast<Real>(27.0) / (pi_3_2 * h3);
			m_l = static_cast<Real>(-18.0) * m_k / m_radius;
			m_W_zero = W(Vector3r::Zero());
		}
...
*/
	...
	{
	protected:
		Real m_radius; // support radius = h in this kernel
		Real m_k;
		Real m_l;
		Real m_W_zero;
	public:
		...
		void setRadius(Real val)
		{
			...
			m_k = static_cast<Real>(165.0) / (32.0 * pi * h3);
			m_l = static_cast<Real>(-1155.0) / (h3*m_radius*4.0*pi);
			m_W_zero = W(Vector3r::Zero());
		}
	...
	public:
		Real W(const Real r)
		{
			...
			if (q >= 0.0 && q <= 1.0)
			{
				res = m_k * pow(static_cast<Real>(1.0) - q, static_cast<Real>(6.0)) * (35.0 * q * q + 18.0 * q + 3);
			}
			return res;
		}
		...
		Vector3r gradW(const Vector3r& r)
		{
			...
			if ((rl > 1.0e-9) && (q <= 1.0))
			{
				...
				res = m_l * q * pow(static_cast<Real>(1.0) - q, static_cast<Real>(5.0)) * (1.0 + 5.0 * q) * gradr;
			}
			else
				res.setZero();
