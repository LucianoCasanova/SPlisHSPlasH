...
{		
	...
	addField({ "velocity_X", FieldType::Scalar, [&](const unsigned int i) -> Real* { return &getVelocity(i)[0]; }, true });
	addField({ "velocity_Y", FieldType::Scalar, [&](const unsigned int i) -> Real* { return &getVelocity(i)[1]; }, true });
	addField({ "velocity_Z", FieldType::Scalar, [&](const unsigned int i) -> Real* { return &getVelocity(i)[2]; }, true });
	addField({ "density", FieldType::Scalar, [&](const unsigned int i) -> Real* { return &getDensity(i); }, false });
	addField({ "mass",	FieldType::Scalar, [&](const unsigned int i) -> Real* {return &getMass(i); }, true });
	addField({ "numberDensity",	FieldType::Scalar, [&](const unsigned int i) -> Real* {return &getNumberDensity(i); }, true});
}
...
FluidModel::~FluidModel(void)
{
	...
	removeFieldByName("velocity_X");
	removeFieldByName("velocity_Y");
	removeFieldByName("velocity_Z");
	removeFieldByName("density");
	removeFieldByName("mass");
	removeFieldByName("numberDensity");
...
void FluidModel::initMasses()
{
	...
	#pragma omp parallel default(shared)
	{
		...
		{
			setMass(i, m_V * m_density0);						// each particle represents a cube with a side length of h		
		}
	}
}
...
void FluidModel::resizeFluidParticles(const unsigned int newSize)
{
	...
	m_numberDensity.resize(newSize);
